...
// FN-5556 call-site guard: packages/engine/src/executor.ts:3990

describe("FN-5556 executor emits session:runtime-resolved", () => {
  let store: TaskStore;
  ...
  it("records executor-purpose runtime resolution event", async () => {
    const auditor = createRunAuditor(store, { runId: "fn-5556-executor", agentId: "executor", taskId: "FN-5556", phase: "execute", source: "executor" });
    ...
    expect(auditRows.some((row) => row.target === "mock" && row.metadata?.sessionPurpose === "executor")).toBe(true);
  });
});
