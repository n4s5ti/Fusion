...
// FN-5556 call-site guard: packages/engine/src/merger.ts:1636

describe("FN-5556 merger emits session:runtime-resolved", () => {
  let store: TaskStore;
  ...
  it("records merger-purpose runtime resolution event", async () => {
    const auditor = createRunAuditor(store, { runId: "fn-5556-merger", agentId: "merger", taskId: "FN-5556", phase: "merge", source: "merger" });
    ...
    expect(auditRows.some((row) => row.target === "mock" && row.metadata?.sessionPurpose === "merger")).toBe(true);
  });
});
