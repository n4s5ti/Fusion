...
// FN-5556 call-site guard: packages/engine/src/triage.ts:1222

describe("FN-5556 triage emits session:runtime-resolved", () => {
  let store: TaskStore;
  ...
  it("records triage-purpose runtime resolution event", async () => {
    const auditor = createRunAuditor(store, { runId: "fn-5556-triage", agentId: "triage", taskId: "FN-5556", phase: "plan", source: "triage" });
    ...
    expect(auditRows.some((row) => row.target === "mock" && row.metadata?.sessionPurpose === "triage")).toBe(true);
  });
});
