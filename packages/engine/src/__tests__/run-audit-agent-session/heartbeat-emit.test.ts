...
// FN-5556 call-site guard: packages/engine/src/agent-heartbeat.ts:2532

describe("FN-5556 agent-heartbeat emits session:runtime-resolved", () => {
  let store: TaskStore;
  ...
  it("records heartbeat-purpose runtime resolution event", async () => {
    const auditor = createRunAuditor(store, { runId: "fn-5556-heartbeat", agentId: "heartbeat", taskId: "FN-5556", phase: "heartbeat", source: "heartbeat" });
    ...
    expect(auditRows.some((row) => row.target === "mock" && row.metadata?.sessionPurpose === "heartbeat")).toBe(true);
  });
});
